...
def test_mnemonic_to_bits():
    ...
    for length in mnemonic.allowed_mnemonic_lengths:
        for _ in range(100):
            ...
            for u, v in zip(test_mnemonic, checksum.bits_to_mnemonic(bitstring)):
                assert u==v
...
def test_checksum_bip39():
    ...
    test_vectors = {'00000000000000000000000000000000':"abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about",
    ...
    for entropy in test_vectors:
        ...
        for u, v in zip(checksum.bits_to_mnemonic(bitstring + checksum_string), test_vectors[entropy].split(' ')):
            assert u==v
